/******************************************************************************
x * Product: iDempiere - sub-project of ADempiere 				              *
 ...
 * 
 */
public class Products2WebStore extends SvrProcess {

	public static final String CLASS_NAME = "cm.itkamer.omnempiere.process.Products2WebStore";
	...
	protected void prepare() {
		...
		for (int i = 0; i < parameters.length; i++) {
			...
			else if (name.equals(I_M_Product_Category.COLUMNNAME_M_Product_Category_ID))
				m_productsCategoryID = parameters[i].getParameterAsInt();

			else if (name.equals(I_M_PriceList_Version.COLUMNNAME_M_PriceList_Version_ID))
				m_priceListVersionID = parameters[i].getParameterAsInt();

			else if (name.equals(I_OmnEmpiereChannel.COLUMNNAME_OmnEmpiere_Channel_ID))
	...
	private String getSingleStationProductsXML(String channel) {
		try {
			...
			for (MProduct product : m_products) {
                ...
                /* Get all locations for product and locator ID */
				...
				writer.writeEndElement();
					...
					writer.writeStartElement("Category_Name");
