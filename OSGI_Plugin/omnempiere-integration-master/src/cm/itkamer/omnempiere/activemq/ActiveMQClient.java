...
package cm.itkamer.omnempiere.activemq;
...
public class ActiveMQClient {
	...
	public ActiveMQClient(String activemqBrokerUrl, String activemqUsername,
	...
	public boolean init() {
		try {
			...
			if (!activemqUsername.isEmpty() && !activemqPassword.isEmpty())
				connection = factory.createConnection(activemqUsername,
						activemqPassword);
			else
			...
			Logger.getLogger(ActiveMQClient.class.getName()).log(Level.SEVERE,
					null, ex);
		}
		return false;
	}
	...
	public boolean sendMessage(Serializable messageObject, String queueName) {
		try {
			...
			ObjectMessage objectMessage = session
					.createObjectMessage(messageObject);
